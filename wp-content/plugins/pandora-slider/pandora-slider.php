...
function pb_insert_slider($atts)
{
    ...
    $slider .=  '<div class="navigation navigation-left">
        ...
        $pb_slider_query = array('post_type' => 'pandora-slide', 'posts_per_page'=>-1);
        $pb_slider_posts = new WP_Query($pb_slider_query);

    if ($pb_slider_posts->have_posts()) { 
        if($ids){
        	foreach ($ids_arr as &$value){
                $slider .= slide_create($value);
            }
        } else {
            while ($pb_slider_posts->have_posts()) {
                $pb_slider_posts->the_post();
