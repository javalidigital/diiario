...
{     
    ...
    if (isset($_POST['block_anchor'])) update_post_meta($post_id, 'block_anchor',  $_POST['block_anchor'] ); 
...
function pb_block_options_meta_box( $post ) { 
	...
	$block_anchor = get_post_meta($post->ID, 'block_anchor', true);
	?>
	...
	<table>
		...
		<tr>
			...
			<td class="w100">
				...
				<input type="text" name="block_anchor" value="<?php echo $block_anchor; ?>" /> 
				- use this link for smooth scroll
			</td>
		</tr>
...
function pb_insert_block($atts)
{
	...
	$pb_block_query = array('post_type' => 'pandora-block', 'posts_per_page'=>-1);
	$pb_block_posts = new WP_Query($pb_block_query);


	if ($id == -1 && $pb_block_posts->have_posts()) {
		while ( $pb_block_posts->have_posts() ) {
			$pb_block_posts->the_post();
			...
			$block_anchor = get_post_meta(get_the_ID(), 'block_anchor', true);
			...
			$pb_block .= '<div class="block '. $block_colored .'" id="'. $block_anchor .'" >';
			$pb_block .=   '<div class="block-container">';
			$pb_block .= 	'<div class="block-thumb">' . get_the_post_thumbnail($post_id, 'full') . '</div>';
